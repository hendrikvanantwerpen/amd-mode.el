...
(setq amd--header-regexp "^define\\s-*(\\s-*\\(\\[[^]]*\\]\\)\\s-*,\\s-*function\\s-*\\(([^)]*)\\)\\s-*{")
...
(defun amd--parse-comma-list (string)
...
(defun amd--find-header ()
  ...
  (if (search-forward-regexp amd--header-regexp nil t)
    (let ((raw-ids (match-string 1))
      ...
      (let ((ids (amd--parse-comma-list raw-ids))
            (vars (amd--parse-comma-list raw-vars)))
